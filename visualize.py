...
def update_progress(current, total, filename, status, brightness, orbit=None):
    progress = {
        ...
        "orbit": orbit.__dict__ if orbit else None
    }
    try:
...
def visualize_fits(fits_file_path, cmap='viridis', norm=LogNorm(), save_fig=False, fig_path=None):
    ...
    try:
        ...
        if save_fig and fig_path:
            ...
            # Extract orbit details for the progress
            orbit_data = {"x": orbit.x.tolist(), "y": orbit.y.tolist()} if orbit else None
            update_progress(
                current=None,  # To be set in the loop
                total=None,    # To be set in the loop
                filename=filename,
                status="Success",
                brightness=brightness,
                orbit=orbit_data
...
def process_all_fits_files(directory, output_video_path, frame_rate=1):
    ...
    images_dir = os.path.join(directory, 'frames')
    ...
    for idx, fits_file in enumerate(fits_files, start=1):
        ...
        try:
            ...
            update_progress(
                ...
                filename=fits_file,
                status="Success",
                brightness=measure_brightness(load_fits_data(fits_path)[0]),
